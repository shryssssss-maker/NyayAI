"""
engine_gemini.py — Primary strategy engine
...
from case_state import CaseState, ActionPlan
...
def _call_with_retry(prompt: str, max_attempts: int = 3) -> str:
    """Call Gemini with exponential backoff. Returns parsed-valid JSON string."""
    last_error = None
...
def run_gemini_engine(state: CaseState) -> ActionPlan:
...
- escalation_path: minimum 3 steps"""
    ...
    try:
        ...
        logger.error(f"[gemini] ActionPlan validation failed: {e}\nRaw: {raw[:500]}")
        raise ValueError(f"Gemini returned invalid ActionPlan: {e}")
